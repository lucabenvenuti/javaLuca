public class Screen {
	...
	public Screen(int screenNumber){		
		for (int i= 0; i<MAXROWS; i++){
			newScreenRowArray[i]=new ScreenRow(LAYOUT[screenNumber][i].toCharArray(), 
					new Category(SYMBOLARRAY[i]));
		}
	}
	...
	public Screen(int screenNumber, Category category, int numberOfSeatToBook, int positionOfSeatToBook){
		...
		this.newScreenRowArray[defineArrayPosition(category)].setBook(numberOfSeatToBook,
	...
	public void setNewScreenRow(int screenNumber, Category category){
		this.newScreenRowArray[defineArrayPosition(category)].setCategory(LAYOUT[screenNumber]
				[defineArrayPosition(category)].toCharArray(), category); 
	}
	
	public boolean setNewScreenRow(int screenNumber, Category category, int numberOfSeatToBook, 
			int positionOfSeatToBook){	
		setNewScreenRow(screenNumber, category);
		boolean ok = this.newScreenRowArray[defineArrayPosition(category)].setCategory(
				LAYOUT[screenNumber][defineArrayPosition(category)].toCharArray(), category, 
				numberOfSeatToBook, positionOfSeatToBook);
		return ok;
	}
	
	public Category setNewScreenRow(int screenNumber, int rowNumber){
		Category category = new Category(defineCategory(rowNumber));
		this.newScreenRowArray[rowNumber].setCategory(LAYOUT[screenNumber][rowNumber].
				toCharArray(), category);
		return category;
	}

	public boolean setNewScreenRow(int screenNumber, int rowNumber, int numberOfSeatToBook, int 
			positionOfSeatToBook){
		Category category = new Category(defineCategory(rowNumber));
	  boolean ok = this.newScreenRowArray[rowNumber].setCategory(LAYOUT[screenNumber][rowNumber].
			  toCharArray(), category, numberOfSeatToBook, positionOfSeatToBook);
	  return ok;
	}
