...
public enum Pos {
	...
	A_1('A', 1), A_2('A', 2), A_3('A', 3), B_1('B', 1), B_2('B', 2), B_3('B', 3), C_1(
			'C', 1), C_2('C', 2), C_3('C', 3);
	 ...
	 * 
	 * @param row
	 *            the character designating the row
	 * @param col
	 *            the integer value designating the column
	...
	public static Pos getPos(char row, int col) throws IllegalArgumentException {
		...
		if (row < 'A' || row > 'C' || col < 1 || col > 3) {
			throw new IllegalArgumentException(
					"Illegal argument to getPos; was " + row + "_" + col);
		}
